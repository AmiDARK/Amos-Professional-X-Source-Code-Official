...
;        Library_Digest.AMOS
;---------------------------------------------------------------------
        Include    "Compact_lib_Size.s"
        Include    "Compact_lib_Labels.s"
...
C_Tk        dc.w     1,0
        ...
        dc.w    L_InSPack7,L_Nul
        dc.b    $80,"I0,0,0,0,0t0",-2
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Lib_Par    InPack6        
; - - - - - - - - - - - - -
    Rbsr    L_PacPar    ; Return A1 = BankID, D1 = ScreenID, D2,D3 = Top, Left ; D4,D5 = Right, Bottom
    Rbsr    L_GetSize
    Rbsr    L_ResBank
    Rbsr    L_Pack
    rts
...
* Screen definition header
    ...
    lea        EcPal(a0),a0
    Move.l     #"AGAP",PsAGAP(a1)
    lea        PsPal(a1),a1
SPac1:
	move.w     EcPalL-EcPal(a0),EcPalL-EcPal(a1) ; 2020.09.09 Update to save RGB12 low bits of picture
    move.w     (a0)+,(a1)+
    dbra       d0,SPac1
    ; *************************** 2019.11.22 copy the AGA 32-255 palette into bank if required. Update 2020.05.01
    moveq      #0,d0
    Move.w     EcNbCol(a2),d0         ; We get the current amonut of colors of the screen
    sub.w      #33,d0                    ; We substract the 32 colors that were already copied.
    bmi        noCopy1                ; If no more colors, jump to .noCopy.
    lea        EcScreenAGAPal(a2),a0  ; Load AGA screen palette to add it to the spack
SPac2:
    move.w     EcScreenAGAPalL-EcScreenAGAPal(a0),EcScreenAGAPalL-EcScreenAGAPal(a1) ; 2020.09.09 Update to save RGB12 low bits of picture
    move.w     (a0)+,(a1)+
    dbra       d0,SPac2
noCopy1:
    move.w     #$FFFF,(a1)
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
;    SPACK ScreenID, Left, Top, Right,Bottom To BankID ************************* 2020.05.01 New Command)
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Lib_Par    InSPack7    
; - - - - - - - - - - - - -
    Rbsr    L_PacPar2
...
;    Unpile parameters
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Lib_Def    PacPar               ; Return A1 = BankID, D1 = ScreenID, D2,D3 = Top, Left ; D4,D5 = Right, Bottom
; - - - - - - - - - - - - -
    move.l    d3,d5        ; Bottom
    move.l    (a3)+,d4     ; Right
    move.l    (a3)+,d3     ; Top
...
;    Unpile parameters
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Lib_Def    PacPar2               ; Return A1 = BankID, D1 = ScreenID, D2,D3 = Top, Left ; D4,D5 = Right, Bottom
; - - - - - - - - - - - - -
    move.l    d3,a1     ; BankID
    move.l    (a3)+,d5     ; Bottom
    move.l    (a3)+,d4     ; Right
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C_Title:
        dc.b     "AMOSPro Picture Compactor AGA V "
        Version
        dc.b    0,"$VER: "
        Version
        dc.b    0
