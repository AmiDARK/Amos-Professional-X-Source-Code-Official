...
NbRain        equ    4
        RsReset
RnDY           rs.w    1               ; = 0 = Nothing on the go
RnFY           rs.w    1
RnTY           rs.w    1               ; = 0 = Nothing on the go
RnBase         rs.w    1
RnColor        rs.w    1               ; Rainbow Color Index (color used to create the Rainbow effect)
RnLong         rs.w    1               ; Rainbow Memory Buffer Size in bytes (Allocated with AllocFastMem)
RnBuf          rs.l    1               ; Rainbow Memory Buffer Pointer (Allocated with AllocFastMem)
RnAct          rs.w    1               ; Rainbow Action.
RnX            rs.w    1               ; Base Position
RnY            rs.w    1               ; Y Position (Shift ?)
RnI            rs.w    1               ; if =-1 Mean nothing to display
RainLong       rs.w    1
        Rb     RainTable,RainLong*NbRain ; Define NbRain RainTable at the size of RainLong
        Rw     RainBow,1
        Rw     OldRain,1
...
; **************** 2020.10.12 New Internal Plugin - Replacement system.
        Rl    ScrnRepVct,1
        Rl    BobRepVct,1
        Rl    SprtRepVct,1
        Rl    CoprRepVct,1
