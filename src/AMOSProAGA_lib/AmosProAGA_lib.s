...
.Loop
    ...
    Rbsr       L_IffRead       ; D1=File, D2=Buffer, D3=Length to read
    beq.s      .Skip
    cmp.l      #12,d0          ; Verify 12 bytes were read
    Rbne       L_DiskError     ; Nope ? Error -> L_DiskError
    ...
    Rbne       L_IffFor        ; If wrong header -> Error IffFor
    ...
    Rbne       L_DiskError     ; -> Error DiskError (file read error, incorrect block size)
...
.SkipIt:                       ; File loading aborded (Simulation)
    ...
    moveq      #0,d3           ; D3 = 0
    Rbsr       L_IffSeek       ; Jump to the beginning of the file
    addq.l     #1,d4           ; D4 +1 = Next Form to read ? (counter)
    subq.l     #1,d7           ; D7 -1 = Decrease max frames amount (decrease counter)
    bne.s      .Loop           ; D7 > 0 -> Jump to .Loop
.End:
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
;                        Load and play frames
;    D5=    IFF disk file dos.library Handle (dos.library/Open)
;    D6=    (Undocumented)
;    D7=    Amount of frames to load/display
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Lib_Def    IffForm
; - - - - - - - - - - - - -
    Rbsr       L_IffFormSize   ; Ask for the frame size 
    add.l      #16,d0          ; D0 = IFF Form buffer size + 16 bytes
    Rjsr       L_ResTempBuffer ; Reserve a temporary buffer for IFF/ILBM form
    beq        .Er             ; On error -> .Er
    move.l     a0,d6           ; D6 = A0 = Temporar memory buffer ?
    Rbsr       L_IffFormLoad   ; Load IFF Frame in the buffer
    cmp.w      d0,d7           ; Compare D0 with D7 (Frames amount)
    Rbne       L_DiskError     ; On error -> L_DiskError
    move.l     TempBuffer(a5),d6 ; D6 = TempBuffer(a5)
    Rbsr       L_IffFormPlay   ; Play/Decode the IFF frame
    moveq      #0,d0           ; D0 = 0
    Rjsr       L_ResTempBuffer ; Release temporary frame buffer
    rts
.Er:
    moveq    #-1,d0            ; Error -1
    rts
...
.FLoop:
    ...
    Rbeq       L_IffFor                ; YES -> Interpret current buffer form.
...
;    Explore the list of chunks (a0)<>(a1)
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
GetIff:
    moveq    #-1,d0
    move.l    (a0),d1
...
* Fabrique l''ecran!
    ...
    Rbcc       L_IllScN
    lea        DefPal(a5),a1
    EcCall     Cree                                         ; **** CREATE SCREEN WHEN LOADING IFF IMAGE
    Rbne       L_EcWiErr 
    move.l     a0,ScOnAd(a5)
...
* Load bit planes
    ...
    move.l     IffFlag(a5),d7          ; D7 = BMHD Loaded ?
    btst       #0,d7                   ; Not loaded ?
    Rbeq       L_FonCall               ; Not Loaded = -> Jump to L_FonCall (Error)
    move.l     BufBMHD(a5),a1
* Regarde si l''image n''est pas + grande que l''ecran!
    move.w     (a1),d5            * Largeur du dessin
    ext.l      d5
    cmp.w      EcTx(a2),d5
    Rbhi       L_IFFWidthTooBig        ; 2020.08.26 Updated to handle new Error Message
    move.w     2(a1),d6        * Hauteur du dessin
    cmp.w      EcTy(a2),d6
    bls        IffB3
    move.w     EcTy(a2),d6
    Rbhi       L_IFFHeightTooBig       ; 2020.08.26 Updated to handle new Error Message

IffB3:
    move.b     8(a1),d7        * Nombre de plans
    ext.w      d7
    cmp.w      #8,d7
    Rbhi       L_IFFDepthTooBig          ; 2020.08.27 Added support to tell that more than 8 bitplanes are not allowed
;    ; ********************************************************* 2020.08.27 Update to handle 18 Bpls for HAM8 (DPaintV) - Start
;    cmp.w      #$18,d7 ; Leads to bad loading as apparently Deluxe Paint V save picture differently.
;    bne.s      IffBz1
;    move.w     #8,d7
;IffBz1:
;    ; ********************************************************* 2020.08.27 Update to handle 18 Bpls for HAM8 (DPaintV) - End
    cmp.w      EcNPlan(a2),d7
    Rbhi       L_IFFDepthTooBig        ; 2020.08.26 Updated to handle new Error Message
    addq.w     #7,d5
    lsr.w      #3,d5
    subq.w     #1,d7
* Enleve le curseur
...
cpyBPLx:
    ...
    rts
; ********************************************** HAM8 Support : Move Bpls 01234567 to 23456701 if Ham8 Is set - End
...
* CAMG chunk
    ...
    beq.s    IfS5
    moveq    #0,d5
    move.l    BufCAMG(a5),a0           ; Graphic Mode used is in this Chunk.
    move.l    (a0),d0
...
;    add.l      #8*2+2+1032,d0           ; 2020.08.30 Updated from 32 colors (64) to 256 colors ("AGAP"+colourAmount+256x*2(Lowbits)+2(Separator)+256*2(HighBits))
    ...
    Rjsr       L_Lst.New               ; Call +AmosProAGA_Loaders.s/Lst.New L1200 / -> A1/D0 = New Element (contains at +0.l previous element memory block pointer, chained list Cur_Banks(a5))
...
.EBank:
    ...
    move.l     d0,a0                   ; A0 = D0
    Rbsr       L_Bnk.EffA0             ; Delete Bank in A0.
.Paeff:
...
; *************************************************************
; * Update all Extensions banks informations                  *
; *************************************************************
    Lib_Def    Bnk.Change
...
;    Rbmi       L_FonCall               ; If yes, -> Jump to error L_FonCall
...
ScOo3:
    ...
    Rbne       L_EcWiErr               ; If screen was not created -> Error
...
; - - - - - - - - - - - - -
    ...
    Rbeq       L_ScNOp                ; No Current Screen -> Jum ScNOp (Error)
clrBuffe:
...
; - - - - - - - - - - - - -
    move.l     d3,-(a3)                ; D3 = Blue Component -> Push to -(a3)
    moveq      #0,d3                   ; D3 = Start Value = 0
...
; - - - - - - - - - - - - -
    moveq    #9,d0
    Rbra    L_GoError
; ************************************************************* 2020.08.26 New Error messages for IFF/ILBM loading issues
    Lib_Def    IFFWidthTooBig
    move.l     #42,d0
    Rbra       L_GoError
;
    Lib_Def    IFFHeightTooBig
    move.l     #43,d0
    Rbra       L_GoError
;
    Lib_Def    IFFDepthTooBig
    move.l     #32,d0
    Rbra       L_GoError
